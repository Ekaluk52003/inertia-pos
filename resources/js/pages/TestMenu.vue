...
const openOptionModal = () => {
  console.log('Opening modal for test item with options:', JSON.stringify(testMenuItem.options, null, 2));
  showOptionModal.value = true;
  selectedOptions.value = [];
  
  // Initialize default selection for required options
  const option = testMenuItem.options[0];
...
const toggleOptionChoice = (optionName, choiceName, price = 0) => {
  ...
  const currentOption = testMenuItem.options.find(opt => opt.name === optionName);
...
<template>
  <Head title="Test Menu" />
  
  <CustomerLayout>
    <div class="container mx-auto py-8 px-4">
      <h1 class="text-2xl font-bold mb-6">Test Menu Item</h1>
  ...
  <div v-show="showOptionModal" 
    ...
    <Card class="w-full max-w-md max-h-[90vh] overflow-y-auto">
      ...
      <CardFooter class="flex justify-between">
        ...
        <Button @click="closeOptionModal">Add to Cart</Button>
      </CardFooter>
    </Card>
  </div>
</template>
