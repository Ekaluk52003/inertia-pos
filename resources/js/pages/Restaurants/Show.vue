...
import { ChefHat, ClipboardList, CreditCard, Menu, QrCode, Settings, Users } from 'lucide-vue-next';
...
const navItems = [
    ...
    { name: 'Payments', icon: CreditCard, route: route('payments.index', props.restaurant.id) },
    ...
    { name: 'Settings', icon: Settings, route: route('restaurants.edit', props.restaurant.id) },
];
</script>
            ...
            <div class="container py-6">
                <div class="mb-6">
                    <div class="flex items-center justify-between">
                        ...
                        <Link :href="route('restaurants.index')" class="text-sm font-medium hover:underline"> Back to All Restaurants </Link>
                    </div>
                </div>

                <!-- Navigation Cards -->
                <div class="mb-6 grid gap-4 md:grid-cols-3 lg:grid-cols-6">
                    <Card v-for="item in navItems" :key="item.name" class="cursor-pointer transition-colors hover:bg-muted/50">
                        <Link :href="item.route" class="block">
                            <div class="flex flex-col items-center justify-center p-4">
                                <component :is="item.icon" class="mb-2 h-6 w-6" />
