...
import { ChefHat, ArrowLeft, CreditCard } from 'lucide-vue-next';
...
const breadcrumbItems = [
  ...
  { title: `Order #${props.order.code.substring(0, 8)}...`, href: route('orders.show', { restaurant: props.restaurant.id, order: props.order.id }), current: true },
];
            ...
            <CardContent>
              <div class="space-y-4">
                ...
                <div class="flex justify-between items-center">
                  ...
                  <span class="px-2 py-1 rounded-full text-xs font-medium" :class="getStatusClass(props.order.status)">
                    {{ props.order.status }}
                  </span>
                </div>
                <div class="flex justify-between items-center">
                  <span class="text-sm font-medium">Payment Status:</span>
                  <span class="px-2 py-1 rounded-full text-xs font-medium" :class="props.order.is_paid ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'">
                  ...
                  <TableRow v-for="item in props.order.orderItems" :key="item.id">
                    <TableCell>
                      <div>
                        ...
                        <div v-if="item.special_instructions" class="text-xs italic text-gray-500 mt-1">
                    ...
                    <TableCell>
                      <div class="flex space-x-2">
                        <form v-if="item.status !== 'served'" :action="route('orders.update-item-status', { restaurant: props.restaurant.id, order: props.order.id })" method="post">
                          <input type="hidden" name="_method" value="patch">
                          <input type="hidden" name="order_item_id" :value="item.id">
                          <input type="hidden" name="status" :value="item.status === 'pending' ? 'cooking' : item.status === 'cooking' ? 'ready' : 'served'">
                          <Button type="submit" size="sm" variant="outline">
                            {{ item.status === 'pending' ? 'Start Cooking' : item.status === 'cooking' ? 'Mark Ready' : 'Mark Served' }}
                          </Button>
                        </form>
                      </div>
                    </TableCell>
